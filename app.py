...
def register():
    if request.method == 'POST':
        ...
        new_user= Users(email=str(email), name=str(name), dob=str(dob), password=generate_password_hash(password, method='sha256'))
...
def createfir():   
    if request.method == 'POST':
        if current_user.is_authenticated :
            ...
            files= os.path.join(r"D:\Hackathon\Hackvengers\Test\temp", file.filename)
